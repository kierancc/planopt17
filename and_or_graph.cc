...
void AndOrGraph::most_conservative_valuation() {
    ...
    */
    ...
    //cout << "Nodes in initial queue: " << queue.size() << endl;

	while (queue.size() > 0) {

		NodeID currentId = queue.front();
		//printf("pop nodeID: %i \n", currentId);
		queue.pop_front();
        AndOrGraphNode *currentNode = &nodes[currentId];
        currentNode->forced_true = true;
        
        //cout << "Current node has " << currentNode->predecessor_ids.size() << " predecessors" << endl;

		for (NodeID predecessorId : currentNode->predecessor_ids) {
			AndOrGraphNode *predecessorNode = &nodes[predecessorId];

            //cout << "Predecessor nodeID: " << predecessorId << endl;

			predecessorNode->num_forced_successors++;

			/* go to next predecessor if already forced true*/
			if (predecessorNode->forced_true == true) {
                //cout << "Skipping already processed nodeID: " << predecessorId << endl;
				continue;
			}

			/* check whether predecessor is now forced true*/
			if (predecessorNode->type == NodeType::OR) {
				/* OR node case */
                //printf("Considering OR nodeID: %i \n", predecessorId);
                //printf("num_forced_successors: %i \n", predecessorNode->num_forced_successors);

				//printf("push OR nodeID: %i \n", predecessorId);
				//predecessorNode->forced_true = true;
				
				queue.push_back(predecessorId);
			} else {
				
				//printf("Considering AND nodeID: %i \n", predecessorId);
				//printf("num_forced_successors: %i \n", predecessorNode->num_forced_successors);
                //printf("successor_ids.size(): %i \n", predecessorNode->successor_ids.size());
                
				if (((unsigned)predecessorNode->num_forced_successors) == predecessorNode->successor_ids.size()) {
					/* AND node case with all succ are forced_true */
					//printf("push AND nodeID: %i \n", predecessorId);

					/* @Kieran: same problem here... */
					//predecessorNode->forced_true = true;
...
void test_and_or_graphs() {
    ...
    */
    ...
    // OUTPUT node name to ID mapping
    // for (const auto &node : ids1) {
    //     cout << "Node name: " << node.first << " has ID: " << node.second << endl;
    // }
